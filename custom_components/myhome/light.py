...
class MyHOMELight(LightEntity):
    ...
    async def async_update(self):
        ...
        if self._attr_supported_features & SUPPORT_BRIGHTNESS:
            await self._gateway_handler.send_status_request(OWNLightingCommand.get_brightness(self._where))
        else:
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if ATTR_FLASH in kwargs and self._attr_supported_features & SUPPORT_FLASH:
            if kwargs[ATTR_FLASH] == FLASH_SHORT:
                return await self._gateway_handler.send(OWNLightingCommand.flash(self._where, 0.5))
            elif kwargs[ATTR_FLASH] == FLASH_LONG:
                return await self._gateway_handler.send(OWNLightingCommand.flash(self._where, 1.5))
        ...
        if ((ATTR_BRIGHTNESS in kwargs or ATTR_BRIGHTNESS_PCT in kwargs) and self._attr_supported_features & SUPPORT_BRIGHTNESS) or (ATTR_TRANSITION in kwargs and self._attr_supported_features & SUPPORT_TRANSITION):
            if ATTR_BRIGHTNESS in kwargs or ATTR_BRIGHTNESS_PCT in kwargs:
                ...
                if _percent_brightness == 0:
                    ...
                    return await self._gateway_handler.send(OWNLightingCommand.set_brightness(self._where, _percent_brightness, int(kwargs[ATTR_TRANSITION]))) if ATTR_TRANSITION in kwargs else await self._gateway_handler.send(OWNLightingCommand.set_brightness(self._where, _percent_brightness))
            else:
                return await self._gateway_handler.send(OWNLightingCommand.switch_on(self._where, int(kwargs[ATTR_TRANSITION])))
        else:    
    ...
    async def async_turn_off(self, **kwargs):
        ...
        if ATTR_TRANSITION in kwargs and self._attr_supported_features & SUPPORT_TRANSITION:
            return await self._gateway_handler.send(OWNLightingCommand.switch_off(self._where, int(kwargs[ATTR_TRANSITION])))
        ...
        return await self._gateway_handler.send(OWNLightingCommand.switch_off(self._where))
