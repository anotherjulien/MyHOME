...
from voluptuous import Schema, Optional, Required, Coerce, Boolean, All, Any, In, Invalid
    ...
    def __call__(self, data):
        ...
        for device in data:
            if CONF_DEVICE_CLASS in data[device]:
                if data[device][CONF_DEVICE_CLASS] in [SensorDeviceClass.POWER, SensorDeviceClass.ENERGY]:
                    if CONF_WHO not in data[device]:
                        ...
                        raise Invalid('invalid sensor class for selected who', path=[device][CONF_WHO])
            ...
            if CONF_WHERE in data[device]:
                _new_key = f"{data[device][CONF_WHO]}-{data[device][CONF_WHERE]}#4#{data[device][CONF_BUS_INTERFACE]}" if CONF_BUS_INTERFACE in data[device] and data[device][CONF_BUS_INTERFACE] is not None else f"{data[device][CONF_WHO]}-{data[device][CONF_WHERE]}"
